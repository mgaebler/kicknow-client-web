...
export default class Venue extends React.Component {
  ...
  handleSubmit(event) {
    ...
    let tables = this.refs.tablesGroup.getCheckedValues()
  ...
  render() {
    ...
    function generateTableSelects(table) {
      return (
        <div className="checkbox">
          {/*<img src={require('file!../../assets/images/logos/' + table.icon)} />*/}
          <label>
    ...
    return (
      <form method="POST" onSubmit={this.handleSubmit}>
        ...
        <div className="form-group" name="tables">
          <label>Welche Tische kann man hier spielen?</label>
          <CheckboxGroup
            ref="tablesGroup"
            value={
              this.props.data.tables ? (
                this.props.data.tables.map(table => table.id)
              ) : (
                []
              )
            }
        ...
        <button className="btn btn-default col-xs-12" type="submit">
          Eintragen
        </button>
      </form>
    )
