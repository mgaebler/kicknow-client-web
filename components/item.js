...
export default class Item extends React.Component {
  ...
  //     'onItemClick': React.PropTypes.function
  //   };
  // }
  ...
  delete(){
    ...
    $.ajax({
      ...
      type: 'POST',
      data: JSON.stringify({
        locationId: this.venue.id
      }),
      success: function (data) {
        // console.log(data);
        if(data.found){
          // todo: if success remove dom element
          this.close();
          this.setState({isDeleted: true});
        } else if (!data.found) {
          console.warn('Data for deletion was not found.');
        }
      }.bind(this),
      error: function (xhr, status, err) {
        console.error('/location/delete', status, err.toString());
      }.bind(this)
    });
  }
  ...
  */
  formatStringTime(timeString){
    var hours = timeString.slice(-4, -2)
  ...
  */
  createRating(num, key){
    let rating = [];
    for(let i=0; i<num; i++) rating.push(<span className="glyphicon glyphicon-heart"></span>);
    return (
      <div key={key}>
        {rating}
      </div>
  ...
  */
  ...
  createTable(table, key){
    let setIcon = function(name){
      ...
      return require('file!../images/logos/' + icon);
    };
    return (
      <img
        key={key}
        src={setIcon(table.name)}
      />
    );
  };
  ...
  renderOpeningHours(open, key){
    return (
        <span key={key}>{formatStringTime(open.start)} - {formatStringTime(open.end)}</span>
    )
  }
  ...
  render() {
    ...
    // console.log(venue)
    let getAddress = function(data){
      let address = data.split(', ');
      return address[0] + ', ' + address[1];
    };

    let getPhotoPath = function(reference){
    ...
    var panelClass = (this.props.selected)? 'card panel panel-info' : 'card panel panel-default'
